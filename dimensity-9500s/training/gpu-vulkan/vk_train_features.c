...
#define NIN     16        // input dim (Fourier features of x) — non-degenerate matmul K
...
int main(void) {
    ...
    for (int i = 0; i < NDATA; i++) {
        ...
        for (int f = 0; f < NIN/2; f++) {            // Fourier features -> K=16 matmul
            xd[i*NIN + 2*f]     = sinf(2.0f*3.14159265f*(f+1)*x);
            xd[i*NIN + 2*f + 1] = cosf(2.0f*3.14159265f*(f+1)*x);
        }
    ...
    int prec_f32 = getenv("PREC") && !strcmp(getenv("PREC"),"f32");
    struct ggml_tensor *mm1 = ggml_mul_mat(cctx, w1, inputs);
    if (prec_f32) ggml_mul_mat_set_prec(mm1, GGML_PREC_F32);
    struct ggml_tensor *h = ggml_relu(cctx, ggml_add(cctx, mm1, b1));
    struct ggml_tensor *mm2 = ggml_mul_mat(cctx, w2, h);
    if (prec_f32) ggml_mul_mat_set_prec(mm2, GGML_PREC_F32);
    struct ggml_tensor *out = ggml_add(cctx, mm2, b2);
