// vk_matmul_bench.c — is there a FAST + CORRECT GPU matmul on the Mali G925?
// Computes C = A^T B (ggml_mul_mat) at a realistic size and reports, per config:
//   - correctness (checksum vs CPU reference; NaN detection)
//   - throughput (GFLOP/s)
// Configs are chosen via env:
//   BACKEND=vk|cpu     (default vk)
//   PREC=f16|f32       (matmul accumulation precision; default f16)
//   GGML_VK_DISABLE_COOPMAT=1  toggles the coopmat matrix-core path
//
// Hypothesis under test: coopmat's *f16 accumulation* overflows (-> NaN); coopmat
// with *f32 accumulation* is both correct and fast.
#include "ggml.h"
...
int main(void){
    ...
    ggml_backend_t backend = use_cpu ? ggml_backend_cpu_init() : ggml_backend_vk_init(0);
    ...
    // deterministic fill; values ~[-1,1], K terms -> dot can reach ~sqrt(K); scale
    // up to stress f16 accumulation range (65504).
    const char *ssel = getenv("SCALE"); float s = ssel ? atof(ssel) : 1.0f;
    // SCALE>1 uses all-positive values so the K-length dot product grows monotonically
    // (~256*s^2 for K=1024) — reliably exceeds the f16 max (65504) to stress f16 accumulation.
    int pos = s > 1.0f;
